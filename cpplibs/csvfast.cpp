...
static int l_read_columns(lua_State* L) {
    ...
    for (int c = 0; c < t->cols; c++) {
        ...
        if (t->is_numeric[c]) {
            ...
            for (int i = 0; i < (int)t->num_cols[c].size(); i++) {
                lua_pushnumber(L, t->num_cols[c][i]);
                lua_rawseti(L, -2, i + 1);
            }
